...
**为什么非要验收员？** 实测约 **1/3** 的无人值守执行结果过不了独立审查——每一份，都是你本来会闭眼直接合进去的代码。[完整故事 →](docs/WHY-A-JUDGE.zh-CN.md)
...
[新手教程](docs/TUTORIAL.zh-CN.md) · [安装](#安装) · [快速上手](#快速上手) · [里面有什么](#里面有什么) · [军团模式](#军团模式legion-mode-多路并行) · [实测](#实测不吹牛) · [区别](#跟同类工具的区别) · [常见问题](#常见问题)
...
一个军团工人做完了一个功能。测试：**全绿。** 代码：**功能完全正确。** 而验收员——全新上下文、没看过计划、只拿到冻结标准和 diff——照样把它毙了：

```
VERDICT: FAIL — NOTES.md modified outside manifest scope src/delta/**
```

它多改了一个清单外的文件。测试全过、代码看着也对，照样被拒——而且**拒对了。** 这就是整个产品的核心：**没有验收的委派，只是许愿。** [为什么"全新上下文验收员"是唯一解 →](docs/WHY-A-JUDGE.zh-CN.md)

**praetor 不是什么：**
- 不是让你写代码"更快"——是让那些"看着像做完了"的委派结果**合不进去**。
- 不是"全自动一把梭"——标准由你拍板，验收员只负责执行它。
- 不是魔法——下方[实测表](#实测不吹牛)**故意留着我们的失败**：4 次早期派活有 2 次卡死、被超时法条击杀。我们没删。

## 为什么要它

**一个统帅，一队士兵。** Fable 5 是你能调到的最强大脑，GPT-5.5 是不知疲倦的高效士兵。praetor 把他们编成一支军队：Claude 拿着地图做所有判断——做什么、收不收——Codex 埋头冲锋执行。大问题能拆的时候，军团模式让**几个士兵同时上、一人啃一块**。

大问题从此不再是"耗时间的问题"。

- **Claude 的额度应该花在判断上，不是搬砖上。** 批量改代码、机械写测试、大面积读代码出报告——这些活烧上下文、烧额度。派给 Codex，用它自己的进程、它自己的额度跑——**你的 Fable 5 额度能撑得远得多，上下文也始终干净留给硬思考。**
- **没有验收的委派只是许愿。** 所以这里没有验收员点头，什么都合不进去。
- **不召唤就蛰伏，召唤了就自主。** 在你没召唤过它的对话里，praetor 一根手指都不动——Claude Code 用起来跟没装一样。说一句（"用codex"）它就上任，整场对话自己分诊、自己拆军团。"这个别派"钉死单个任务，"先别派活了"直接卸任，`STOP` 文件全线停火。
...
从这一句起，它就任整场对话的指挥——真正的机械累活（16 个文件批量改名、机械写测试、大面积读代码出报告）praetor 自己分诊、自己派发：
...
<p align="center"><img src="docs/assets/lifecycle.zh.svg" alt="派发生命周期——召唤一次、标准冻结进git、Codex执行、验收员判卷，PASS则提交、FAIL则重试直至大声接管" width="100%"></p>
...
- **dispatching-legion** —— 2–5 路工人并行（git 工作树隔离、可碰清单、按序合并、强制集成验收）
- **writing-codex-briefs** —— 自包含派工单 + 真能保护你的验收标准（红→绿检查、退出码、清单）
- **codex-judge** —— 全新上下文验收员：没看过计划、亲自跑命令、判决不可推翻

**硬保证**
- **不召唤就蛰伏** —— 召唤后整场对话自主分诊、每次动手前必声明；"这个别派"钉死任务、"先别派活了"卸任、`STOP` 全线停火
- **标准冻结进 git** —— "怎么算干完"在 Codex 出场前就写死，且防篡改
- **大声接管** —— 不存在静默失败这条路
- **git 状态边界** —— Codex 只改文件、永不碰 git；`.git` 只读是设计
- **零配置** —— 中转站自动尊重；实在要调只有两个环境变量

**执行层（v0.4）—— 法条用代码强制，不靠自觉**
- **praetor-dispatch** —— 有 STOP 文件、标准没冻结或被改过、第 4 次重试，一律拒绝发射；看门狗到点击杀挂起的执行。重试上限记在磁盘上——**上下文被压缩也数得清**
- **praetor-verdict** —— 验收员自己的笔：落判决前先机械核对冻结标准的哈希；标准被动过就记 **TAMPERED**，绝不记 PASS
- **提交闸门** —— 一个钩子物理拦截 `codex/*` 分支上的 `git commit` / `git merge`：没有验收员的 PASS 记录、或标准哈希对不上，就是提交不进去。"FAIL 谁也推翻不了"从承诺变成了**拒绝**
- **praetor-manifest-check** —— 军团开拔前机械证明各路清单互不相交（静态包含 + 文件级双检）
- 全部实弹测过：32 断言的测试套件拿假执行器把每条法条打一遍，CI 每次推送都跑 —— [tests/harness.test.sh](tests/harness.test.sh)
...
每行一类任务——派 vs 不派——耗时与验收判决（数据以英文版为准，两版同步更新）：
...
首批数据 n=1——重复积累后换中位数。全盘托出：**4 次早期派活有 2 次卡死**被超时法条击杀，两次重试均成功。派活有固定开销（开分支+冻结+验收）：**小活派了反而慢**，skill 会直说而不是硬派。

## 跟同类工具的区别

Claude↔Codex 桥不止我们一家，各有所长。但没有第二家同时做到这四件事：**统帅级大脑自由调度高效士兵——问题能拆就并行上——每份写入结果独立验收，你的高价额度只花在判断上。**（只读分析类派活按设计不过验收员——闸门是为"合并"设的；那类报告由 Claude 自己复核。） 摆事实：
