...
在**一个 git 仓库里**打开 Claude Code（整个流程靠分支隔离——那是你的后悔药）。v0.3 起 praetor 会自动分诊——你正常安排活，够格的机械块它自己声明一句就派。本教程用明说的方式走一遍，让每一步都看得见（自动模式跑的是完全相同的步骤，只是先声明再动手）：
...
v0.3 起 praetor 凭自己的判断派活（永远先声明）——下面这些常驻刹车就是你的方向盘：
