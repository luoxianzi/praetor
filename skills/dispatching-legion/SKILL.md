...
description: Use when praetor has been summoned this conversation (see dispatching-to-codex) AND a job splits into 2–5 genuinely independent, mechanical, checkable pieces with provably disjoint file footprints — the split is Fable's own call to detect and make; no extra permission beyond the summon. Announce the muster table in ONE message, then run the legion. Also triggers on an explicit ask that names Codex ("send these to codex in parallel", "派几路codex一起干"), which counts as a summon — a bare "run these in parallel" without naming Codex is NOT a summon (that's a request for ordinary parallel subagents, e.g. superpowers' dispatching-parallel-agents). Standing brakes always win: plain-language veto or a STOP file halts everything. The real gate is disjointness: when in doubt about the partition, serialize with dispatching-to-codex instead.
---
...
**0. Muster.** Build the lane table. With **superpowers** installed, decomposition **defaults** to `superpowers:writing-plans` — each plan task with exact file paths + a concrete acceptance check becomes one lane. Without superpowers, write it inline. Each lane needs: a one-line goal, exact pass/fail commands, and a **may-touch manifest** (explicit paths/globs). Verify the manifests are **pairwise disjoint** with the mechanism, not your eyes: write them to a file (one `<lane> <glob>` per line) and run `praetor-manifest-check <file>` — it catches both static containment (`src/**` swallowing `src/beta/**`) and any tracked file matching two lanes; a non-zero exit means fix the partition or serialize. Then still judge whether each declared set is semantically complete (shared allocators, SSoT modules, transitive imports) — when unsure, serialize. Cap at **5 lanes**; more independent tasks → waves of ≤5, each wave fully judged and merged before the next. **Freeze the integration bar as a file, not a memory:** write `.codex/INTEGRATION.md` (union of all lanes' checks + the repo's whole-tree gates) and commit it to the base branch NOW, at Muster — same tamper rules as a lane bar; the integration judge later reads the file and refuses checks recited from your context (which may have compacted by then). **Run the whole-tree gates on the base commit first** — if the repo is already red before any lane exists, the integration verdict is unwinnable and no lane is to blame; fix or surface that before mustering. **Announce the muster table** (Lane | task | may-touch | acceptance + the one-line why: "4 lanes, disjoint, ~15 min vs ~50 serial") and proceed — the announcement is the consent surface; any plain-language veto or a STOP file stops the legion cold. The real gate is disjointness, not permission.
...
**4. Dispatch ∥N.** Launch all lanes as parallel background tasks through the harness wrapper, each rooted in its worktree via `-C`, brief via quoted stdin heredoc (never interpolate):
```
praetor-dispatch -C ../<repo>-legion/<dispatch-id>/<lane> <<'PRAETOR_BRIEF'
<self-contained brief for this lane>
PRAETOR_BRIEF
```
Per lane, the wrapper enforces the same laws as a single dispatch — STOP, frozen bar, on-disk attempt counter, watchdog timeout — and prints `session-id: <id>` (stderr lands in the lane's untracked `.codex/codex.err`, **never `2>/dev/null`**; the reasoning stream stays out of your context). `PRAETOR_MODEL`/`PRAETOR_EFFORT`/relay overrides apply to all lanes identically.
...
**6. Judge (sequential).** After each lane finishes, spawn a fresh `codex-judge` subagent pointed at that lane's worktree + branch. It runs the frozen checks, the tamper check, AND the manifest check (any file outside MANIFEST → FAIL), then records its verdict via `praetor-verdict` in the lane worktree — the commit gate reads that record per lane. No parallel judges in v0.2. FAIL → re-brief via `praetor-dispatch -C <lane> --resume <that lane's session-id>` with the heredoc stdin rule. Per-lane cap 2 retries (the wrapper refuses more). **Legion-wide retry budget = N+1 total** across all lanes; when it's spent, remaining FAIL lanes go straight to takeover — no resume. **Partition-smell brake:** if 2+ lanes FAIL their first attempt, stop retrying everything and offer to serialize — the partition is likely wrong, not the workers.

**7. Merge (ordered, after every lane has a final verdict).** For each PASS lane: planner commits the work in its worktree (Codex never commits) — **add the manifest paths only (`git add <manifest globs>`), never `git add -A`**: executor-side tool noise (e.g. `.serena/`, `.codex/codex.err`) must not enter the repo (live-test finding). Then drop `.codex/ACCEPTANCE.md` from the branch (`git rm .codex/ACCEPTANCE.md && git commit -m "praetor: drop acceptance bar"`), then merge lanes into the base branch in the declared order. **Any textual merge conflict = proof of a manifest breach that slipped both gates → `git merge --abort` immediately (never leave the base mid-merge — a half-merged tree blocks the stash restore and everything after), then halt all merging: loud report naming which lanes are already merged and which are not, never auto-resolve.** Already-merged lanes stay pending until an integration verdict — run the integration judge on just the merged set, or revert them; never call them done unjudged.

**8. Integrate (mandatory).** First make the merged base tree runnable: install any deps a lane's checks introduced (an env failure at this gate blames the environment, not a lane — the judge will name it). Then spawn one fresh integration `codex-judge` on the merged tree: the integration bar is the **frozen `.codex/INTEGRATION.md` committed at Muster** — the judge tamper-checks it and refuses a bar recited from memory. This is the only gate that catches "each lane green alone, red together". FAIL → revert the last-merged lane, re-run to bisect the culprit, revert it, report it as FAIL. A legion is never "done" until integration PASSes. After the verdict, drop the bar from the base branch (`git rm .codex/INTEGRATION.md && git commit -m "praetor: drop integration bar"`) so `.codex/` never ships.
