...
3. **Brief + dispatch.** Self-contained brief (Codex sees zero chat history). Dispatch through the harness wrapper — it enforces the laws in code, not vibes (STOP at the repo root, bar committed-frozen-and-single-commit, attempt counter on disk that survives context compaction, hard timeout watchdog). Pass the brief via **stdin with a quoted heredoc** — never interpolate it into the command line (quotes/backticks in task text would break the shell or inject):

   ```
   praetor-dispatch <<'PRAETOR_BRIEF'
   <the brief>
   PRAETOR_BRIEF
   ```

   The wrapper routes Codex's stderr to the untracked `.codex/codex.err` (**never `2>/dev/null`** — the session id you need for retries is printed on stderr; live-test finding) and prints `session-id: <id>` for you. It refuses to launch when a law would be broken: exit 3 = STOP file, 4 = bar not frozen (or amended since dispatch), 5 = retry budget exhausted (Law 3 — take over now). Analysis mode is `praetor-dispatch --analysis` (read-only sandbox; snapshots the tree and reports exactly what a run changed, touching nothing itself). If `praetor-dispatch` is somehow not on PATH (plugin bin not loaded), fall back to raw `codex exec -m gpt-5.5 -c model_reasoning_effort="xhigh" --sandbox workspace-write - 2>.codex/codex.err` with the same heredoc — and then every law above is yours to keep by hand.

   Model/effort overrides, in priority order: `PRAETOR_MODEL` / `PRAETOR_EFFORT` env vars (used verbatim) → active relay provider in `~/.codex/config.toml` (flags dropped, config respected) → stock default. The wrapper applies this order itself. Never `danger-full-access`. Timeout override: `PRAETOR_TIMEOUT` seconds (default 600).
   **Hard timeout:** the wrapper's watchdog kills a hung executor at the deadline (exit 124) — never unbounded; the attempt was already counted on disk before launch, so even a kill -9 consumes it. After a timeout: record `A-TIMEOUT-KILLED` in the verdict history, reset the tree (`git checkout -- . && git clean -fd -e .codex` — safe here: the write path always starts from a clean or stashed tree on a throwaway branch), and retry **with a fresh full dispatch, never `--resume`** (the reset erased the work the old session believes exists); a timeout consumes one of the 2 retries (Law 3).
   **Any other non-zero exit** (crash, auth death) counts exactly the same — the wrapper says so on stderr: read `.codex/codex.err` for the reason, reset the tree the same way. If the error is auth/login-shaped, stop retrying — tell the user to run `codex login` (stale credentials pass preflight's file-presence fallback; the real error only surfaces here). There is no error exit that silently re-dispatches outside the retry counter — attempt 4 is refused by the wrapper itself.
...
4. **Judge.** Spawn a FRESH subagent using the plugin's `codex-judge` agent + repo path + branch name. Codex's work is UNCOMMITTED — the judge reviews the working tree (`git diff HEAD`), runs every frozen check, checks the bar wasn't moved, and **records its verdict itself via `praetor-verdict`** (which mechanically re-verifies the bar's blob hash before writing — an amended bar records TAMPERED, never PASS). Returns PASS or FAIL + evidence. Without that recorded PASS, the commit gate will physically block step 5.
5. **Resolve.** PASS → you commit (Codex never commits). The harness gate enforces Law 2 here: on a `codex/*` branch, `git commit` and `git merge` are physically blocked unless the judge recorded a PASS whose bar hash still matches — a FAIL is not overridable by anyone, including you. A PASS certifies the WHOLE working tree the judge saw, so bind the commit to it: run `git diff HEAD --name-only` and stage exactly that list — **never `git add -A`** (executor-side tool noise must not enter the repo), and never a from-memory subset (dropping one needed file ships a branch that fails the very check the PASS certified). After committing, `git status --porcelain` must show nothing but known tool noise. Then report in plain language. FAIL → re-brief via the same stdin rule as step 3 (never interpolate the fix text into the command line):

   ```
   praetor-dispatch --resume <session-id> <<'PRAETOR_BRIEF'
   <the re-brief: judge's reasons + what to fix, same frozen bar>
   PRAETOR_BRIEF
   ```

   Use the session id the wrapper printed at dispatch; if you did not capture one, send a fresh full dispatch instead — **never `resume --last`** (it resumes the machine's most recent Codex session and can hijack an unrelated task). Max 2 retries — the wrapper refuses attempt 4 on its own — then loud takeover.
